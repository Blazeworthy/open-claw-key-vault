...
- **On macOS, `security add-generic-password` takes the value as an argument**,
  ...
  query Keychain directly anyway. The documented `-w` prompt reads the
  controlling terminal rather than stdin, so it cannot replace this
  non-interactively. Documented rather than hidden.
